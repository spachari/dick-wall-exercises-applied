...
class Module03 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  class ComplexNum(val real : Double, val imaginary : Double) {
    ...
    def +(other : ComplexNum) = new ComplexNum(real + other.real, imaginary + other.imaginary)

    //def +(other : Double) = new ComplexNum(real + other, imaginary) method overloading way

    override def toString: String = if (imaginary > 0.0) s"$real + ${imaginary}i" else s"$real - ${scala.math.abs(imaginary)}i"

    implicit def myApply(i : Double) = new ComplexNum(i)
  }
  ...
  object ComplexNum {
    ...
    implicit def myApply(i : Double) = new ComplexNum(i) //implicit way
  }
