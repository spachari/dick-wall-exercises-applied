...
class Module05 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Multiply variable number of arguments") {
    ...
    def multiplyDoubles(doubles : Double*) = {
      def multiply(listOfDoubles : List[Double], startingValue : Double = 1.0) : Double = {
        listOfDoubles match {
          case x :: xs => multiply(xs, startingValue * listOfDoubles.head)
          case Nil => startingValue
        }
      }
      multiply(doubles.toList, 1.0)
    }
  ...
  test ("Recurse with varargs") {
    ...
    /*
    def listOfLists(args : String*) = {
      def recursiveListCreator(list : List[String]) : List[List[String]] = {
        list match {
          case x :: xs => list :: recursiveListCreator(xs)
          case Nil => List(list)
        }
      }
      recursiveListCreator(args.toList).reverse.tail.reverse
    }
    */
    
    def listOfLists(args : String*) : List[List[String]] = {
      if (args.isEmpty) Nil else
        args.toList :: listOfLists(args.tail : _*)
    }
  ...
  test ("URL cleaner") {
    def urlClean(urlAsString : String) : URL = urlAsString match {
      case x if (x.startsWith("http://")) => new URL(urlAsString)
      case x => new URL("http://" + x.replace(":malformed", ".com").replace(":",""))
    }
